...
Your goal is to build a lightweight, highly maintainable, and robust system consisting of a Python-based GitHub Action (`weaviate-test-reporter`) and a Next.js frontend, all powered by Weaviate as the vector database.
...
* **Parsing:** Standard `junit.xml` via `junitparser`. `action/src/weaviate_test_reporter/parser/core.py` stays generic; handling specific to one producer (e.g. gotestsum) goes in its own module under `parser/dialects/` (see the README section "Adding a JUnit dialect").
