...
Relationship: each TestCase has a cross-reference 'belongsToRun' to its parent TestRun. Filter TestCase DIRECTLY — no cross-reference — for time windows (run_started_at) and for version_minor / job_name / branch: all four are denormalized onto the case. Only fields that live SOLELY on TestRun require going through belongsToRun to TestRun — repository, actor, trigger_type, run_id, the run URLs, and the finer version fields version_patch / version_full. To count or list the tests within a run, traverse from TestRun to its TestCases, or read the run's tests_* counts directly.
